...
### 0단계: 모호성 감지 → prompt-clarify 자동 발동

**실행 시작 전**, 아래 3가지 항목을 즉시 판단한다.

| 항목 | 불명확 판단 기준 |
|------|----------------|
| 목적/목표 | 기대하는 결과물이 무엇인지 특정할 수 없음 |
| 출력 형식 | 파일·코드·보고서·UI 등 결과 형태가 미정 |
| 범위/대상 | 어떤 대상에 어느 범위까지 작업할지 불분명 |

**판정 규칙:**

```
불명확 항목 수 ≥ 2개  →  prompt-clarify 먼저 실행 후 1단계 진행
불명확 항목 수 ≤ 1개  →  prompt-clarify 생략, 1단계 바로 진행
```

**자동 제외 조건** (아래 중 하나라도 해당하면 모호성 판단 없이 1단계 직행):
- `/prompt-clarify` 직접 호출인 경우
- task-orchestrator가 갭 스킬 생성을 위해 내부 호출된 경우
- 이전 단계에서 이미 prompt-clarify가 실행된 경우 (중복 방지)

**prompt-clarify 실행 시 흐름:**

```
[0단계] prompt-clarify 실행 (Solo: 직렬 / Team: 직렬 — 반드시 결과 확인 후 진행)
        ↓
  사용자 답변 수렴 완료
        ↓
  구체화된 프롬프트를 입력으로 → 1단계 진행
```

> **실행 계획 출력 시**: 0단계가 발동된 경우 표에 `| 0 | prompt-clarify | 모호성 해소 | 원본 요청 | 구체화된 프롬프트 | 직렬 |` 행을 추가한다.
...
## 예시 — Team Mode 실행 계획 (모호성 감지 있음)

```
## 실행 계획  [Team Mode]

### [0단계] 모호성 감지 → prompt-clarify 발동
불명확 항목: 출력 형식, 범위/대상 (2개)

| 순서 | 스킬 | 역할 | 입력 | 예상 출력 | 실행 방식 |
|------|------|------|------|-----------|-----------|
| 0    | prompt-clarify | 모호성 해소 | 원본 요청 | 구체화된 프롬프트 | 직렬 |

### [본 작업]
| 순서 | 스킬 | 역할 | 입력 | 예상 출력 | 실행 방식 |
|------|------|------|------|-----------|-----------|
| 1    | rss-fetcher + gemini-client + web-dev | 뉴스 웹앱 구현 | 구체화된 프롬프트 | index.html + app.js | 직렬 |
...
갭 스킬: 없음
→ 0단계 prompt-clarify 실행 후 본 작업을 시작합니다.
```

## 예시 — Team Mode 실행 계획 (모호성 없음, 직행)
